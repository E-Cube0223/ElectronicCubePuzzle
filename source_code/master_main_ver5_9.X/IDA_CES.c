...
char Create_shortest_command_depp_N2(char check_depth)
{
	char return_num;
	return_num = 0;
	if (shortest_command[check_depth] == IDA_LAST_COMAND)
	...
	if (check_depth >= 1 && check_depth < shortest_depth)
	{
		if (shortest_command[check_depth] == shortest_command[check_depth - 1]) //CE_IDA_LIST[shortest_command[check_depth]].name[0]
		{
			Create_shortest_command_depp_N2_2(check_depth);
			return 2;
		}
	}
	return return_num;
...
//é}êÿé¿çs
char Create_shortest_command_depp_N2_2(char check_depth)
{
	char return_num;
	return_num = 0;
